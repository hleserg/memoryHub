...
| `core/models/experience.py` | Lived experience, key moments, reframing, session closure metadata (E22.7: `close_reason`, `restart_reason`, `agent_recap`) | `SessionExperience`, `KeyMoment` (with `id: UUID`), `FeltSense`, `ContextHalo`, `ReframingNote`, `EmotionalDepth`, `ReframingNoteAppendResult` |
...
| `core/ports/state_store.py` | Storage for experience/identity/narrative/key moments | `StateStore`, `ExperienceQuery`, `SessionExperienceQuery`, `ValuesTouchedQuery`, `DepthQuery`, `DateRangeQuery`, `FactRefsContainsQuery`; KeyMoment CRUD: `create_key_moment`, `list_key_moments`, `get_key_moment` |
