...
| `core/services/maintenance_worker.py` | Claim and dispatch maintenance jobs (salience decay, memory guardian scan, **mREBEL relation extraction** and **deeper linguistic enrichment** per HLE-28, **reflection-overload cadence check** per HLE-30) from `MaintenanceQueue`; missing optional dependencies → `mark_skipped` with reason instead of crashing | `MaintenanceWorker` |
...
| `adapters/storage/file_state_store.py` (`FileStateStore`) | `StateStore` | JSON files (experience + identity + narrative + eigenstate) + `key_moments.jsonl`; **v2**: `list_key_moments(session_id)` filtering support; `get_key_moment` falls back to legacy JSONL and backfills the per-moment index; session sorting normalizes legacy naive + UTC-aware timestamps |
