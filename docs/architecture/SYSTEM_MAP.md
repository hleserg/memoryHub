...
| `adapters/agent/deps.py` (`AtmanDeps`, `AtmanDeps.from_config`) | — | frozen DI container wiring `SessionManager`, `IdentityService`, `ExperienceService`, `MicroReflectionService`, `StateStore`; `from_config` factory transfers validated limits from `AgentConfig`; optional `injected_context` field for `system_prompt` injection mode; **R11.7/R12** optional `pending_review_inbox` and `reflection_request_queue` fields gating tool registration; **opt-2** optional `passive_memory_injector` field |
...
| `adapters/agent/factory.py` (`build_deps`) | — | Assembles `AtmanDeps`, `SessionManager`, `FileStateStore`, services, optional `AffectDetector` from workspace + `AgentConfig`; bootstraps missing identity/narrative for new runner workspaces and wires `SessionManager(workspace=...)` so crash journals are active; **opt-2**: conditionally instantiates `PassiveMemoryInjector` (embedding + factual_memory + state_store) when `ATMAN_LINGUISTIC_ENABLED=true`; failure is non-fatal (logs warning, RAG stays disabled) |
