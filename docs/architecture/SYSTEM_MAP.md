...
| `core/ports/affect.py` (HLE-52) | Async affect hook for `SessionManager.record_event`; concrete `AffectDetector` lives in `atman.affect/` and is injected via this port | `AffectPort` (runtime_checkable Protocol): `process(text, *, thinking, session_id)`, `submit_self_report(report, *, session_id)`; `AppendKeyMomentFn` callback type |
| `core/ports/state_store.py` | Storage for experience/identity/narrative/eigenstate/key moments; **v2**: extended with sessions API (`create_session`, `get_session`, `update_session`, `list_recent_sessions`) and standalone KeyMoment API (`store_key_moment` idempotent upsert, `mark_moment_accessed`, `update_moment_structured_markers`, `find_moments_by_entity`); **HLE-59**: native ranged query `list_sessions_in_range(agent_id, start, end)` so the Reflection Engine no longer has to fetch a capped window and filter client-side (default impl delegates to `list_recent_sessions(limit=10_000_000)` for adapters without an override) | `StateStore` (with `create_key_moment`, `store_key_moment`, `list_key_moments`, `get_key_moment`, `mark_moment_accessed`, `update_moment_structured_markers`, `find_moments_by_entity`, `create_session`, `get_session`, `update_session`, `list_recent_sessions`, `list_sessions_in_range`), `ExperienceQuery`, `SessionExperienceQuery`, `ValuesTouchedQuery`, `DepthQuery`, `DateRangeQuery`, `FactRefsContainsQuery` |
...
| `core/ports/memory_guardian.py` | Memory quality scanning and finding persistence | `MemoryGuardian` (ABC): `scan_orphan_entities`, `scan_merge_candidates`, `scan_stale_moments`, `scan_embedding_gaps`, **HLE-31** `scan_quality_metrics` (Level-C psychological: `affect_detector_silent`, `divergence_pattern`, `stance_formation_too_fast`), **HLE-32** `inline_check_fact` / `inline_check_entity` / `inline_check_key_moment` (row-level post-write checks, default no-op), `write_finding`, `get_unresolved`, `resolve_finding` |
...
| `core/services/session_manager.py` | Session runtime: start, `record_event` (optional async **AffectDetector** hook + **value refusal auto-recording**), `append_key_moment` / `append_key_moment_input`, finish with eigenstate (thread-safe registry, optional `max_active_sessions`, optional `affect_workspace` + `AffectDetectorConfig`, **optional `workspace` for JSONL session journals, inter-process journal locks, and orphan recovery**, **silent refusal detection via `RefusalDetectorConfig`**); **v2**: persists `Session` row via `state_store.create_session` at start and `update_session` at finish (best-effort, debug fallback for legacy stores); **HLE-27**: optional `post_write_scheduler: PostWriteScheduler` — fires mREBEL / lingvo enrichment jobs via `MaintenanceQueue` from `finish_session` (after `create_key_moment` + `store_key_moments` make moments durable in `state_store`, so the worker's `get_key_moment` cannot miss them); scheduler errors are caught and logged so a queue outage cannot break session finish; **HLE-29**: optional `linguistic_analyzer: LinguisticAnalyzer`, `divergence_detector: DivergenceDetector`, `divergence_event_store: DivergenceEventStore` are accepted on the constructor and forwarded into the legacy `AffectDetector` construction path so the divergence pipeline persists events for R6 aggregator (post-HLE-52 the canonical wiring path is `attach_affect(AffectDetector(...))` from the composition root); **HLE-56**: tracks pending `AffectDetector` fire-and-forget tasks per session and `finish_session` drains them (5 s timeout) before flipping `is_finished=True` so late-firing affect hooks can still call `append_key_moment` without raising `SessionAlreadyFinishedError`; **HLE-57**: `_recover_orphaned_sessions` now calls `state_store.store_key_moments(session_id, loaded_moments)` after `create_experience`, so recovered sessions are visible to `get_key_moments_for_session` and therefore to Daily/Deep reflection (previous behavior silently excluded them forever); **HLE-32**: optional `inline_validator: InlineValidator` — when wired, `finish_session` runs `validator.check_key_moment` on each persisted moment so lightweight findings (e.g. `affect_detector_silent` info) appear in `validation_findings` within milliseconds of the originating write | `SessionManager`, `MAX_EIGENSTATE_ITEMS`; session errors live in `core/exceptions.py` |
...
| `core/services/session_experience_view.py` (R3) | Bridge helper that synthesises a virtual `SessionExperience` from a `Session` + `list[KeyMoment]` so `ReflectionModel` prompts keep working after `DailyReflectionService` migrated off `ExperienceRepository`; obsolete once `ReflectionModel` consumes `(Session, moments)` directly; **HLE-58**: `experience.id = deterministic_session_experience_id(session.id)` — matches what `finish_session` writes to `StateStore`, so `ReflectionEvent.experiences_analyzed` references stay resolvable via `state_store.get_experience(exp.id)` instead of orphaning at audit time | `build_session_experience` |
...
| `core/services/merge_candidates_handler.py` (R10) | Deep-reflection service: resolve `similar_entities` validation findings via LLM merge decision. On confirm, invokes `EntityRegistry.merge_entities` (FK rewrite owned by the registry) and resolves the finding `fixed`; on reject, resolves `ignored` with the LLM's reason. Guards against double-merge if a prior pass committed but `resolve_finding` failed (skips re-merging when the drop entity is already `needs_disambiguation=True` — leftover-finding cleanup). Errors leave the finding unresolved for the next pass — never half-merge. | `MergeCandidatesHandler`, `MergeOutcome` |
...
| `core/services/maintenance_worker.py` | Claim and dispatch maintenance jobs (salience decay, memory guardian scan, **mREBEL relation extraction** and **deeper linguistic enrichment** per HLE-28, **reflection-overload cadence check** per HLE-30) from `MaintenanceQueue`; missing optional dependencies → `mark_skipped` with reason instead of crashing; mREBEL entity lookup uses extracted entity type to avoid same-name graph corruption | `MaintenanceWorker` |
...
| `core/services/inline_validator.py` (HLE-32) | Fire-and-forget post-write row-level validation; wraps `MemoryGuardian.inline_check_*` in try/except + auto-`write_finding` so a single bad row never blocks the write hot path. Three entry points: `check_fact`, `check_entity`, `check_key_moment` | `InlineValidator` |
| `core/services/ambient_memory_service.py` (HLE-33) | Entity-anchor parallel RAG: parses anchors via `LinguisticAnalyzer`, resolves each biographical anchor (person_ref / topic / location / object_ref) to a registry entity, then fans out `find_moments_by_entity` + `find_facts_by_entity` + `get_current_stance` and merges results. Stances precede episodes (plan §7), moments ordered by salience-aware composite `salience*0.4 + intensity*0.3 + recency*0.3` (plan §8), token-budget cap, `mark_accessed` on returned moments (plan §12 шаг f). All backend calls wrapped in try/except so a flaky adapter degrades silently | `AmbientMemoryService`, `AmbientSurfaceItem`, `AmbientResult` |
...
| `affect/detector.py` | Language sniff, trigger logic (anomaly / random sample / divergence / linguistic / self-report), `KeyMoment` writes via callback; **v2**: optional `LinguisticAnalyzer` DI for structured markers enrichment; **v3**: linguistic divergence signals fold into `reasons` with `TriggerReason.LINGUISTIC` so cold-start sessions with only linguistic signals get accurate `trigger_reason`; **HLE-29**: optional `divergence_detector: DivergenceDetector` + `divergence_event_store: DivergenceEventStore` — when both are wired, each `AgentMessageAnalysis` is fed through the detector and resulting `DivergenceEvent` rows are persisted (with `agent_id` + `session_id` provenance) for R6 aggregator to consume; failures inside detect/write are logged but never abort message processing | `AffectDetector`, `AffectDetectorConfig`; CLI `python -m atman.affect.detector --demo` |
...
| `core/clock_impl.py` | `SystemClock` (default `ClockPort`, zero-dep stdlib wrapper) + `ensure_utc(dt)` pure timezone normaliser; `FrozenClock` test-fixture lives in `adapters/clock.py` (HLE-54) |
...
| `adapters/storage/in_memory_state_store.py` (`InMemoryStateStore`) | `StateStore` | full in-memory implementation with deep copies; **v2**: sessions dict + standalone key_moments + `store_key_moment` (idempotent upsert), `mark_moment_accessed`, `update_moment_structured_markers`, `create_session`/`get_session`/`update_session`/`list_recent_sessions`; session sorting normalizes legacy naive + UTC-aware timestamps |
| `adapters/storage/file_state_store.py` (`FileStateStore`) | `StateStore` | JSON files (experience + identity + narrative + eigenstate) + `key_moments.jsonl`; **v2**: `list_key_moments(session_id)` filtering support; `update_moment_structured_markers` persists lingvo markers to per-moment files, JSONL, and any matching per-session bundle; `get_key_moment` falls back to legacy JSONL and backfills the per-moment index; session sorting normalizes legacy naive + UTC-aware timestamps |
...
| `adapters/memory/in_memory_memory_guardian.py` (`InMemoryMemoryGuardian`) | `MemoryGuardian` | scan_orphan_entities + scan_merge_candidates + scan_stale_moments + scan_embedding_gaps + finding lifecycle (write/get_unresolved/resolve); **HLE-31** also implements `scan_quality_metrics` — sliding-window checks for `affect_detector_silent` (incomplete_coloring rate), `divergence_pattern` (same divergence_type ≥ N over window via optional `DivergenceEventStore`), `stance_formation_too_fast` (stance formed < 24h after backing moments via optional `EntityStanceStore`). Missing optional inputs ⇒ sub-scan silently emits zero findings. **HLE-32** also implements the inline post-write checks: fact missing `metadata['embedding']` → `embedding_missing`; entity without embedding (non-value/principle) → `embedding_missing`; KeyMoment with `incomplete_coloring=True` → `affect_detector_silent` (severity `info`). All inline checks dedupe against unresolved findings for the same `(target_table, target_id, finding_type)` |
...
| `adapters/clock.py` (`FrozenClock`) (HLE-54) | `ClockPort` | Test-fixture frozen clock; the default `SystemClock` lives in `core/clock_impl.py` because it is a zero-dep stdlib wrapper |
...
| `adapters/observability/logging_overload_alert_sink.py` (HLE-30) | `ReflectionOverloadAlertSink` | routes alerts through standard library `logging` at CRITICAL/WARNING level; structured payload (severity + details) attached via `extra` for JSON formatters; logging failures swallowed |
| `adapters/observability/composite_overload_alert_sink.py` (HLE-30) | `ReflectionOverloadAlertSink` | fan-out wrapper over an ordered tuple of sinks; per-sink exceptions suppressed so one flaky downstream cannot block the rest |
...
| `adapters/agent/deps.py` (`AtmanDeps`, `AtmanDeps.from_config`) | — | frozen DI container wiring `SessionManager`, `IdentityService`, `ExperienceService`, `MicroReflectionService`, `StateStore`; `from_config` factory transfers validated limits from `AgentConfig`; optional `injected_context` field for `system_prompt` injection mode; **R11.7/R12** optional `pending_review_inbox` and `reflection_request_queue` fields gating tool registration; **opt-2** optional `passive_memory_injector` field; **WP-08 v2** optional `skill_manager: SkillManagerPort | None = None`; **HLE-29** optional `divergence_event_store: DivergenceEventStore | None = None` exposing the persisted divergence stream so R6 DivergenceAggregator (and ad-hoc readers) can consume it without reaching into `SessionManager` internals; **HLE-30** optional `reflection_overload_monitor: ReflectionOverloadMonitor | None = None` + `overload_alert_inspect: InMemoryOverloadAlertSink | None = None` (the in-memory tap inside the composite sink, surfaced for admin UI / debug reads without poking the monitor's private `_sink._sinks` chain); **HLE-32** optional `memory_guardian: MemoryGuardian | None = None` — the same guardian instance fed into the inline pipeline, exposed so cli_maintenance / admin endpoints read findings (`get_unresolved`) without rebuilding the chain; **HLE-33** optional `ambient_memory: AmbientMemoryService | None = None` — entity-anchor parallel RAG, callers reach `compose_injection(text, agent_id=...)` directly through deps |
...
| `adapters/agent/factory.py` (`build_deps`, `_build_skill_manager`) | — | Assembles `AtmanDeps`, `SessionManager`, `FileStateStore`, services, optional `AffectDetector` from workspace + `AgentConfig`; bootstraps missing identity/narrative for new runner workspaces and wires `SessionManager(workspace=...)` so crash journals are active; **HLE-27**: assembles `InMemoryMaintenanceQueue` + `PostWriteScheduler` and passes the scheduler into `SessionManager` so finished sessions fan out mREBEL / lingvo enrichment jobs onto the queue — the queue accumulates until an out-of-process `python -m atman.cli_maintenance run --loop` consumes it (production: swap to `PostgresMaintenanceQueue` + dedicated worker pod); **HLE-29**: unconditionally builds `NoOpLinguisticAnalyzer` + `DivergenceDetector(agent_id)` + `InMemoryDivergenceEventStore` and threads them into the `AffectDetector` constructed via `session_manager.attach_affect(...)` so the divergence pipeline persists events out of the box (real GLiNER/mREBEL analyzer slots in later via the `linguistic` extra); **opt-2**: conditionally instantiates `PassiveMemoryInjector` (embedding + factual_memory + state_store + zero-dep `BM25EmbeddingAdapter` for RRF fusion + `NoOpLinguisticAnalyzer` always + `BgeReranker` / `NoOpReranker` fallback) when `ATMAN_LINGUISTIC_ENABLED=true`; `NoOpLinguisticAnalyzer` is always wired so ambient_mode is reachable in lean deployments; `BgeReranker` is tried first (requires FlagEmbedding + model assets), falls back to `NoOpReranker`; failure of the whole block is non-fatal (logs warning, RAG stays disabled); **WP-08 v2**: builds `SkillManager` via `_build_skill_manager()` BEFORE `MicroReflectionService` so the reflection hook (`process_session_skills`) actually fires — passes it both to `MicroReflectionService(skill_manager=…)` and `AtmanDeps`; `_build_skill_manager` tries `PostgresSkillStore(db_url, agent_id=agent_id)` first, falls back to `InMemorySkillStore` when PostgreSQL is unavailable (no-PostgreSQL local dev), and finally returns `None` if neither store can be constructed |
...
| `skills/models.py` | Domain models (frozen dataclasses + StrEnums) | `SkillKind`, `SkillStatus`, `SkillOrigin`, `Skill`, `SkillInvocation`, `SuggestionStrength`, `SkillSuggestion`, `DailySkillSummary`, `DeepSkillSummary` (HLE-36 hook returns); `Skill.is_pinned`, `Skill.description_short` properties |
...
| `skills/port.py` | `SkillManagerPort` Protocol (runtime_checkable) | `SkillManagerPort`: 11 methods — `list_pinned`, `list_available`, `trigger_router`, `invoke`, `mark_result`, `capture`, `get_skill`, `process_session_skills`, `write_session_skills_marker` (HLE-35), `process_daily_skills` (HLE-36), `process_deep_skills` (HLE-36) |
| `skills/noop.py` | Silent no-op implementation for disabled mode | `SkillsDisabledError`, `NoopSkillManager` (read-only methods return empty; write methods raise `SkillsDisabledError`; `process_session_skills`, `write_session_skills_marker`, `process_daily_skills`, `process_deep_skills` are silent no-ops returning empty/None) |
| `skills/store.py` | `SkillStore` Protocol — storage interface | `SkillStore`: `save_skill`, `get_skill_by_name`, `get_skill_by_id`, `list_pinned`, `list_by_status`, `list_active_on_demand`, `list_by_revision_needed` (HLE-36), `update_skill_status`, `update_pinning`, `update_stats`, `bump_sessions_since_use`, `set_revision_needed`, `reset_sessions_since_use`, `create_invocation`, `set_preliminary_status`, `write_agent_marker`, `append_behavioral_hint`, `append_user_feedback_hint`, `get_unprocessed_invocations`, `set_final_status`, `mark_processed` |
| `skills/in_memory_store.py` | In-memory `SkillStore` implementation (tests) | `InMemorySkillStore` |
| `skills/postgres_store.py` | PostgreSQL `SkillStore` over `public.skills` + `public.skill_invocations` (psycopg3 + dict_row + RLS); **bound to a single `agent_id` at construction** (same pattern as `PostgresEntityStanceStore`) — every method routes through `_conn()` which sets `atman.current_agent` so `FORCE ROW LEVEL SECURITY` lets the query through; mismatch between bound and passed `agent_id` raises `ValueError` | `PostgresSkillStore(db_url, agent_id=...)` |
...
| `skills/manager.py` | Real `SkillManager` implementation | `SkillManager`: full lifecycle — invoke (subprocess entry script, preliminary_status), mark_result, capture (creates SKILL.md + entity + row), `process_session_skills` (finalise invocations, update stats, auto-pin/downgrade, mark processed), `write_session_skills_marker` (HLE-35: writes `atman_session_skills_<ts>.json` to workspace), `process_daily_skills` (HLE-36: bump revision_priority for idle pending skills, list high-priority candidates), `process_deep_skills` (HLE-36: read-only classification — archive candidates by idle threshold, problematic by failure rate) |
| `skills/text_parser.py` | HLE-37 fallback: parse agent response text for implicit skill mentions | `InvocationTextParser(store, min_confidence=0.7, max_per_call=5)`; `parse(text, agent_id, session_id) -> list[InferredInvocation]` (pure), `parse_and_record(...)` (creates invocations with `agent_marker='unclear'`); scoring: exact name (1.0), kebab→underscore/space variant (0.9), ≥2 keyword overlap (0.9), single keyword (0.75); word-boundary match prevents over-firing |
| `skills/installer.py` | HLE-38 install-external installer (pure) | `install_external(source, agent_id, *, store, agents_root, name_override=None, dry_run=False, http_get=None)`; sources: local dir / local `.zip` / HTTPS `.zip`; validates manifest, refuses zip-slip + plain HTTP + bare GitHub URLs; `SkillInstallError`, `InstallResult` |
| `skills/revision.py` | HLE-40 skill revision workflow | `SkillReviser` Protocol + `NoopSkillReviser` default; `SkillRevisionProposal`, `RevisionOutcome`; `SkillRevisionService(store, reviser).revise_pending(agent_id, max_skills=3, dry_run=False)` — backs up `SKILL.md` as `SKILL.md.bak.<UTC-ts>`, rewrites body, bumps minor version, clears `revision_needed`; `_bump_minor_version(version)` semver helper |
...
| `skills/cli.py` | CLI skill management (entry point `atman-skills`) — uses Rich via `atman.term` (`console`, `print_ok`/`print_err`/`print_warn`/`print_help_text`, `Panel`, `Table`); no raw `print()` per AGENTS.md | `list`, `show`, `disable`, `enable`, `pin`, `unpin`, `archive`, `inspect-invocations`, `force-revise`, `install-external` (HLE-38: local path / `.zip` / HTTPS), `revise` (HLE-40); read-only commands work when `enabled=false` |

**DB migration**: `migrations/versions/0015_skills.sql` — `public.skills` (RLS by `agent_id`, indexes on status + pinning, `description TEXT NOT NULL DEFAULT ''` column persists the human-readable summary used by bootstrap injection and agent tools) + `public.skill_invocations` (RLS, partial index on `processed_at IS NULL`); always created regardless of `skills.enabled`.

**Config (HLE-36)**: `SkillsSettings.daily_revision_idle_bump_sessions` (default 5), `deep_archive_sessions` (default 50), `deep_failure_rate_threshold` (default 0.5), `deep_min_invocations_for_failure_rate` (default 5).

**Env override (HLE-41)**: `ATMAN_SKILLS_ENABLED=false|0|no|off` short-circuits `factory._build_skill_manager`, takes precedence over `settings.skills.enabled`.
...
| `src/atman/skills/cli.py` | CLI | Skill management: `list`, `show`, `disable`, `enable`, `pin`, `unpin`, `archive`, `inspect-invocations`, `force-revise`, `install-external` (HLE-38), `revise` (HLE-40); read-only commands work when `skills.enabled=false`; entry point `atman-skills` |
...
| `DailyReflectionService` / `DeepReflectionService` ↔ `ReflectionRequestQueue` (R12 drain) | `core/services/reflection_service.py` → `core/ports/reflection_request_queue.py` | optional queue arg; on each live `reflect()` `take_pending(level)` drains pending agent-driven requests; reasons surface in the LLM context (`agent_requested_focus`) and `ReflectionEvent.key_insight`/`notes`; `mark_consumed` after the event is durably persisted. Replay paths (idempotent re-runs) do not re-drain, so requests submitted between runs reach the next live job. |
...
| `DailyReflectionService` / `DeepReflectionService` ↔ `EntityStanceFormulator` (R7) | `core/services/entity_stance_formulator.py` → `core/ports/entity_stance.py` (`EntityStanceStore`), `core/ports/entity_registry.py` (`EntityRegistry.get_entity` / `list_entities`), `core/ports/state_store.py` (`find_moments_by_entity`), `core/ports/reflection.py` (`ReflectionModel.formulate_entity_stance` + `SYSTEM_PROMPT_STANCE`) | optional hook (needs `agent_id`); Daily `formulate_for_new_entities` writes a new provisional stance per entity with ≥ `DEFAULT_MIN_MOMENTS` (5) moments — old stance auto-superseded (never deleted), `based_on_moment_ids` mandatory (§9); Deep `revise_stale` re-formulates stances older than `DEFAULT_STALENESS_DAYS` (30) against new moments — material valence delta (≥ `STANCE_MATERIAL_CHANGE_THRESHOLD`=0.2) writes a new stance, no-change promotes the existing to non-provisional and bumps `confidence` by `CONFIDENCE_REAFFIRM_BUMP` (0.1). Stance is interpretation, not aggregation — empty `stance_text` from the LLM is treated as "decline" and skipped. |
...
| `DeepReflectionService` ↔ `EntityRelationsFormulator` (R9) | `core/services/entity_relations_formulator.py` → `core/ports/entity_relation_store.py` (`EntityRelationStore`), `core/ports/entity_registry.py`, `core/ports/state_store.py` (`find_moments_by_entity`), `core/ports/reflection.py` (`ReflectionModel.formulate_entity_relation` + `SYSTEM_PROMPT_ENTITY_RELATION`) | optional hook (needs `agent_id`); builds a co-occurrence index over `KeyMoment`s from the entity registry, asks the LLM per pair with co-occurrence ≥ `DEFAULT_MIN_COOCCURRENCES` (3) for a typed relation, and persists confirmed relations (`confidence ≥ DEFAULT_MIN_CONFIDENCE`=0.7) as `learned_by='reflection'`. Coexists with real-time mREBEL extraction — store dedups by `(agent, pair, type, learned_by)`. |
| `DeepReflectionService` ↔ `MergeCandidatesHandler` (R10) | `core/services/merge_candidates_handler.py` → `core/ports/memory_guardian.py` (`get_unresolved` / `resolve_finding`), `core/ports/entity_registry.py` (`get_entity` / `merge_entities`), `core/ports/state_store.py` (`find_moments_by_entity`), `core/ports/reflection.py` (`ReflectionModel.decide_entity_merge` + `SYSTEM_PROMPT_ENTITY_MERGE`) | optional hook (needs `agent_id`); for each unresolved `similar_entities` finding (R8 deferred), asks the LLM with ≤ `DEFAULT_CONTEXT_LIMIT` (5) recent KeyMoments per side. On confirm: `EntityRegistry.merge_entities` (FK rewrite owned by registry) + `resolve_finding(fixed)`. On reject with reason: `resolve_finding(ignored, note=reason)`. On any error (LLM/store/registry) or empty reason: leaves the finding unresolved for the next Deep pass — never half-merge. Keep-side chosen by the LLM's `canonical_name` hint, falling back to higher `mention_count`. |
| `DeepReflectionService._propose_identity_revision` ↔ R5/R7/R8/R9/R10 outcomes (R11) | `core/services/reflection_service.py` | R11 identity-level synthesis: in addition to the existing pattern-driven `potential_habit`/`potential_principle` and low-`health.overall_score` rules, the proposal now reads the structured-marker pattern descriptions (`growth_indicator=progress/regression`, `agency_level=high/low`) and the outcomes from R7 stance revise, R9 relations formulation, R10 merge, R8 triage. Output stays a free-form text proposal — actual writes go through R11.5 `IdentityService.apply_self_change` + `SelfAppliedChangeStore` audit. |
...
| `MaintenanceWorker` ↔ `MaintenanceQueue` + `SalienceDecayService` + `MemoryGuardian` + (HLE-28) `StateStore` + `EntityRelationExtractor` + `EntityRelationStore` + `EntityRegistry` + `LinguisticAnalyzer` + (HLE-30) `ReflectionOverloadMonitor` | `core/services/maintenance_worker.py` → `core/ports/maintenance_queue.py`, `core/ports/salience_decay.py`, `core/ports/memory_guardian.py`, `core/ports/state_store.py`, `core/ports/entity_relations.py`, `core/ports/entity_relation_store.py`, `core/ports/entity_registry.py`, `core/ports/linguistic.py`, `core/services/reflection_overload_monitor.py` | DI; `run_once()` claims batch and dispatches jobs; every enrichment / monitor dependency is optional — missing ⇒ `mark_skipped(reason=…)` instead of crashing |
...
| Connection | Files | Type |
|-----------|-------|------|
| `AtmanDeps` ↔ `SessionManager`, `IdentityService`, `ExperienceService`, `MicroReflectionService`, `StateStore` | `adapters/agent/deps.py` | DI container (frozen dataclass); optional `injected_context` for `system_prompt` memory injection mode; **WP-08 v2** optional `skill_manager: SkillManagerPort | None`; **HLE-29** optional `divergence_event_store: DivergenceEventStore | None` (same instance fed into the affect pipeline, so readers see whatever was written); **HLE-30** optional `reflection_overload_monitor: ReflectionOverloadMonitor | None` + `overload_alert_inspect: InMemoryOverloadAlertSink | None`; **HLE-32** optional `memory_guardian: MemoryGuardian | None` (shared with the inline validator); **HLE-33** optional `ambient_memory: AmbientMemoryService | None` |
...
- ✅ Legacy `KeyMoment` JSONL-only rows in `FileStateStore` — `get_key_moment` scans `key_moments.jsonl`, skips malformed lines, and backfills `{id}.json`; covered by `tests/test_file_state_store.py::test_get_key_moment_loads_legacy_jsonl_and_backfills_index_file` and `test_search_experiences_uses_legacy_jsonl_moment_fallback`.
- ✅ Mixed legacy naive + UTC-aware session timestamps — store-level `list_recent_sessions` sorting and repository range filtering normalize to UTC; covered by `tests/test_file_state_store.py::test_list_recent_sessions_sorts_mixed_legacy_naive_and_aware_timestamps` and `tests/test_state_store_session_repository.py::test_get_sessions_in_range_normalizes_legacy_naive_session_timestamps`.
...
| PR #572 follow-up | `build_deps` constructed `MicroReflectionService` BEFORE `skill_manager`, so `self._skill_manager` was always `None` and `process_session_skills` was dead code; `runner.py` also did not pass `agent_id` to `micro_reflection.reflect`, so even after fixing factory order the hook would not fire | covered (`tests/test_skill_factory_wiring.py::test_build_deps_passes_skill_manager_to_micro_reflection`, `tests/test_skill_factory_wiring.py::test_build_deps_falls_back_to_in_memory_skill_store_when_postgres_unavailable`) |
| PR #572 follow-up | `PostgresSkillStore` write/update methods used raw `psycopg.connect(db_url)` without setting `atman.current_agent`, so `FORCE ROW LEVEL SECURITY` silently dropped all updates/inserts/selects | covered (`tests/test_skill_factory_wiring.py::test_postgres_skill_store_rejects_cross_agent_access`); review of every method confirms all queries flow through `_conn(agent_id)` |
| PR #572 follow-up | Migration `0015_skills.sql` had no `description` column even though `Skill.description` is used by bootstrap injection and agent tools, so descriptions were silently lost after a PostgreSQL roundtrip | covered (schema change + `tests/test_skill_factory_wiring.py::test_skills_migration_persists_description_column`) |
| PR #572 follow-up | `skills/cli.py` used raw `print()` despite AGENTS.md mandating Rich for CLI entrypoints | fixed: CLI now uses `atman.term` (`console`, `print_ok`/`print_err`/`print_warn`/`print_help_text`, `Panel`, `Table`); covered (`tests/test_skill_factory_wiring.py::test_skills_cli_uses_rich_not_raw_print`) |
| PR #572 follow-up | PyYAML was imported at module top-level in `skills/manifest.py` without being a declared dependency, so `atman-skills show` would crash if pyyaml was missing transitively | fixed: `pyyaml>=6.0` added to `[project].dependencies` |
| PR #572 follow-up | `skills.enabled=True` default required PostgreSQL, causing a warning on every `build_deps` for the no-external-service local path | fixed: `_build_skill_manager` falls back to `InMemorySkillStore` when PostgreSQL is unavailable (`tests/test_skill_factory_wiring.py::test_build_deps_falls_back_to_in_memory_skill_store_when_postgres_unavailable`) |
| current PR | `list_recent_sessions` in file/in-memory stores could raise `TypeError` before repository UTC normalization when legacy naive and current aware timestamps coexisted | fixed: storage sort keys use `ensure_utc`; covered by `tests/test_file_state_store.py::test_list_recent_sessions_sorts_mixed_legacy_naive_and_aware_timestamps` and `tests/test_state_store_session_repository.py::test_get_sessions_in_range_normalizes_legacy_naive_session_timestamps` |
| HLE-27/HLE-28/HLE-30 follow-up | File-backed lingvo jobs could be marked succeeded without persisting `structured_markers`; mREBEL could bind relations to a same-name entity of the wrong type; PostgreSQL `maintenance_jobs` rejected `reflection_overload_check` | covered (`tests/test_post_write_wiring.py::test_worker_lingvo_persists_structured_markers_in_file_store`, `tests/test_post_write_wiring.py::test_worker_mrebel_resolves_entities_by_detected_type`, `tests/test_state_store_contract.py::test_update_moment_structured_markers_does_not_raise`, `tests/test_postgres_migration_security.py::test_maintenance_job_migrations_accept_all_job_names`) |
...
| Legacy KeyMoment JSONL fallback + mixed session timestamp sorting | ✅ closed | `tests/test_file_state_store.py`, `tests/test_state_store_session_repository.py` |
