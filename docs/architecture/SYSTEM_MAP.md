...
| `skills/cli.py` | CLI skill management (entry point `atman-skills`) — uses Rich via `atman.term` (`console`, `print_ok`/`print_err`/`print_warn`/`print_help_text`, `Panel`, `Table`); no raw `print()` per AGENTS.md | `list`, `show`, `disable`, `enable`, `pin`, `unpin`, `archive`, `inspect-invocations`, `force-revise`; read-only commands work when `enabled=false` |
