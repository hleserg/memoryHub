...
**2. Контекстуальный ореол (`context_halo`)**
...
Новое поле в Experience Store: `context_halo: string` — атмосферный контекст момента в свободной прозе.
...
- Субъективный слой с `emotional_valence`, `depth`, `values_touched`
...
При каждом обращении к воспоминанию (в т.ч. через Ambient Layer) — обновляется `access_count`, предотвращается `salience` decay. Активно используемые воспоминания не затухают.
