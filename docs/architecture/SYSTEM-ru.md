...
Atman использует LLM для двух отдельных целей, каждая с независимой конфигурацией:

### Внутренний LLM Atman (ReflectionModel)

Используется для рефлексии, рефрейминга, обнаружения паттернов, обновления нарратива и оценки здоровья.

**Конфигурация через переменные окружения:**

\`\`\`bash
# Выбор бэкенда (по умолчанию: openai)
ATMAN_REFLECTION_BACKEND=openai      # или: anthropic, mock

# Для OpenAI-совместимых endpoints (бэкенд по умолчанию)
...
# Для Anthropic (когда ATMAN_REFLECTION_BACKEND=anthropic)
ANTHROPIC_API_KEY=sk-ant-...
ATMAN_ANTHROPIC_MODEL=claude-opus-4-7
ATMAN_ANTHROPIC_MAX_TOKENS=1024

# Для offline / CI (когда ATMAN_REFLECTION_BACKEND=mock)
# Конфигурация не требуется — использует детерминированный mock
\`\`\`

**Опции бэкендов:**

\* \`openai\` — Универсальный OpenAI-совместимый endpoint (llama-server, vLLM, Ollama с OpenAI compat и т.д.)
\* \`anthropic\` — Anthropic Claude API (пока не реализовано, используйте \`openai\` или \`mock\`)
\* \`mock\` — Детерминированные ответы на основе шаблонов для тестирования (без сетевых вызовов)

**Реализация:**

\* Порт: \`atman.core.ports.reflection.ReflectionModel\`
\* Адаптеры: \`OpenAIReflectionModel\`, \`MockReflectionModel\`
\* Фабрика: \`atman.adapters.reflection.get_reflection_model()\`

### Тестовый агент Pydantic AI (инфраструктура разработки)

Отдельный агент, который действует как тестовый пользователь, говорящий \*\*с\*\* Atman. Не часть Atman — часть инфраструктуры разработки/тестирования.

**Конфигурация через переменные окружения:**

\`\`\`bash
AGENT_LLM_BASE_URL=http://localhost:8080/v1   # llama-server с Gemma4
AGENT_LLM_MODEL=gemma4
AGENT_LLM_API_KEY=dummy                        # llama-server игнорирует это
\`\`\`

**Реализация:**

\* Модуль: \`agent/\` (отдельно от \`src/atman/\`)
\* Конфигурация: \`agent.config.AgentLLMConfig\`
\* Фабрика: \`agent.atman_agent.create_agent()\`

**Эти два LLM-подключения полностью независимы:**

\* Внутренний LLM Atman на \`:8081\` делает рефлексию/рефрейминг/обнаружение паттернов
\* LLM агента на \`:8080\` — это тестовый пользователь, который говорит с Atman
\* Они могут указывать на разные модели/endpoints
\* Они никогда не взаимодействуют друг с другом напрямую
