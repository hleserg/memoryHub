...
| `core/services/maintenance_worker.py` | Забор и диспетчеризация задач обслуживания (salience decay, memory guardian scan, **mREBEL relation extraction**, **lingvo enrichment** — HLE-28) из `MaintenanceQueue`; отсутствующие опциональные зависимости → `mark_skipped` с причиной вместо краша | `MaintenanceWorker` |
