...
| `core/models/experience.py` | Прожитый опыт, ключевые моменты, переосмысление, метаданные завершения сессии (E22.7: `close_reason`, `restart_reason`, `agent_recap`) | `SessionExperience`, `KeyMoment` (с полем `id: UUID`), `FeltSense`, `ContextHalo`, `ReframingNote`, `EmotionalDepth`, `ReframingNoteAppendResult` |
...
| `core/ports/state_store.py` | Хранилище опыта/identity/нарратива/key moments | `StateStore`, `ExperienceQuery`, `SessionExperienceQuery`, `ValuesTouchedQuery`, `DepthQuery`, `DateRangeQuery`, `FactRefsContainsQuery`; CRUD для KeyMoment: `create_key_moment`, `list_key_moments`, `get_key_moment` |
