...
| `core/ports/affect.py` (HLE-52) | Асинхронный хук аффекта для `SessionManager.record_event`; конкретный `AffectDetector` живёт в `atman.affect/` и инжектится через этот порт | `AffectPort` (runtime_checkable Protocol): `process(text, *, thinking, session_id)`, `submit_self_report(report, *, session_id)`; `AppendKeyMomentFn` тип колбэка |
| `core/ports/state_store.py` | Хранилище опыта/identity/нарратива/eigenstate/ключевых моментов; **v2**: расширен sessions API (`create_session`, `get_session`, `update_session`, `list_recent_sessions`) и API самостоятельных KeyMoment (`store_key_moment` — идемпотентный upsert, `mark_moment_accessed`, `update_moment_structured_markers`, `find_moments_by_entity`); **HLE-59**: нативный ranged-запрос `list_sessions_in_range(agent_id, start, end)` — Reflection Engine больше не фильтрует client-side с ограниченным окном (дефолтная реализация в порту делегирует `list_recent_sessions(limit=10_000_000)` для адаптеров без override) | `StateStore` (с `create_key_moment`, `store_key_moment`, `list_key_moments`, `get_key_moment`, `mark_moment_accessed`, `update_moment_structured_markers`, `find_moments_by_entity`, `create_session`, `get_session`, `update_session`, `list_recent_sessions`, `list_sessions_in_range`), `ExperienceQuery`, `SessionExperienceQuery`, `ValuesTouchedQuery`, `DepthQuery`, `DateRangeQuery`, `FactRefsContainsQuery` |
...
| `core/ports/memory_guardian.py` | Сканирование качества памяти и персистенс находок | `MemoryGuardian` (ABC): `scan_orphan_entities`, `scan_merge_candidates`, `scan_stale_moments`, `scan_embedding_gaps`, **HLE-31** `scan_quality_metrics` (Level-C психологические: `affect_detector_silent`, `divergence_pattern`, `stance_formation_too_fast`), **HLE-32** `inline_check_fact` / `inline_check_entity` / `inline_check_key_moment` (row-level пост-write проверки, default no-op), `write_finding`, `get_unresolved`, `resolve_finding` |
...
| `core/services/session_manager.py` | Сессионный runtime: старт, `record_event` (опциональный async **AffectDetector** + **авто-запись ценностных отказов**), `append_key_moment` / `append_key_moment_input`, завершение с eigenstate (потокобезопасный реестр, опциональный `max_active_sessions`, опционально `affect_workspace` + `AffectDetectorConfig`, **опционально `workspace` для JSONL-журналов сессий, межпроцессных journal-locks и orphan recovery**, **тихая детекция отказов через `RefusalDetectorConfig`**); **HLE-27**: опциональный `post_write_scheduler: PostWriteScheduler` — ставит mREBEL / lingvo enrichment задачи в `MaintenanceQueue` из `finish_session` (после `create_key_moment` + `store_key_moments`, когда моменты уже видны воркеру через `state_store.get_key_moment` и job не уйдёт в permanent `skipped`); ошибки scheduler перехватываются и логируются — обвал очереди не ломает finish сессии; **HLE-29**: опциональные `linguistic_analyzer: LinguisticAnalyzer`, `divergence_detector: DivergenceDetector`, `divergence_event_store: DivergenceEventStore` принимаются конструктором и пробрасываются в legacy-путь создания `AffectDetector`, чтобы divergence-pipeline персистил события для R6 aggregator (после HLE-52 каноничный путь — `attach_affect(AffectDetector(...))` из composition root); **HLE-56**: трекинг fire-and-forget задач `AffectDetector` по сессии и drain (таймаут 5 с) в `finish_session` до выставления `is_finished=True` — поздние affect-хуки успевают вызвать `append_key_moment` без `SessionAlreadyFinishedError`; **HLE-57**: `_recover_orphaned_sessions` после `create_experience` вызывает `state_store.store_key_moments(session_id, loaded_moments)`, чтобы recovered-сессии стали видны `get_key_moments_for_session` и попали в Daily/Deep reflection (раньше тихо исключались навсегда); **HLE-32**: опциональный `inline_validator: InlineValidator` — когда подключён, `finish_session` зовёт `validator.check_key_moment` на каждый персистированный момент, и lightweight findings (например `affect_detector_silent` info) появляются в `validation_findings` в миллисекундах после write | `SessionManager`, `MAX_EIGENSTATE_ITEMS`; ошибки сессий в `core/exceptions.py` |
...
| `core/services/merge_candidates_handler.py` (R10) | Deep-рефлексия: резолвит `similar_entities` findings через LLM merge decision. Confirm → `EntityRegistry.merge_entities` (FK rewrite на стороне registry) + finding fixed; reject → ignored с reason от LLM. Защита от double-merge: если предыдущий проход уже мёрджил, но `resolve_finding` упал, текущий проход не вызывает `merge_entities` повторно (drop-entity уже `needs_disambiguation=True`) — только закрывает leftover finding. Любая ошибка оставляет finding unresolved — never half-merge. | `MergeCandidatesHandler`, `MergeOutcome` |
...
| `core/services/maintenance_worker.py` | Забор и диспетчеризация задач обслуживания (salience decay, memory guardian scan, **mREBEL relation extraction**, **lingvo enrichment** — HLE-28, **reflection_overload_check** — HLE-30) из `MaintenanceQueue`; отсутствующие опциональные зависимости → `mark_skipped` с причиной вместо краша; mREBEL lookup учитывает извлечённый тип сущности, чтобы не портить граф одноимёнными сущностями другого типа | `MaintenanceWorker` |
...
| `core/clock_impl.py` | `SystemClock` (дефолтный `ClockPort`, zero-dep обёртка над stdlib) + `ensure_utc(dt)` — чистый нормализатор временной зоны; `FrozenClock`-фикстура живёт в `adapters/clock.py` (HLE-54) |
...
| `affect/detector.py` | Определение языка, триггеры (аномалия / random sample / расхождение thinking↔сообщение / linguistic / self-report), запись `KeyMoment` через callback; **v2**: опциональный `LinguisticAnalyzer` через DI для обогащения structured markers; **v3**: лингвистические сигналы расхождения попадают в `reasons` с `TriggerReason.LINGUISTIC`, чтобы cold-start-сессии с только лингвистическими сигналами получали корректный `trigger_reason`; **HLE-29**: опциональные `divergence_detector: DivergenceDetector` + `divergence_event_store: DivergenceEventStore` — когда оба подключены, каждый `AgentMessageAnalysis` прогоняется через детектор и `DivergenceEvent`-записи персистятся (с `agent_id` + `session_id` для provenance) для последующего чтения R6 агрегатором; ошибки detect/write логируются, но не ломают обработку сообщения | `AffectDetector`, `AffectDetectorConfig`; CLI `python -m atman.affect.detector --demo` |
...
| `adapters/storage/in_memory_state_store.py` (`InMemoryStateStore`) | `StateStore` | полная реализация в памяти; **v2**: словарь сессий + самостоятельные key moments + `store_key_moment` (идемпотентный upsert), `mark_moment_accessed`, `update_moment_structured_markers`, сессионные методы; сортировка сессий нормализует legacy naive + UTC-aware timestamp |
| `adapters/storage/file_state_store.py` (`FileStateStore`) | `StateStore` | JSON-файлы (опыт + identity + нарратив + eigenstate) + `key_moments.jsonl`; **v2**: фильтрация по `session_id` в `list_key_moments`; `update_moment_structured_markers` сохраняет lingvo-маркеры в per-moment файл, JSONL и совпадающий session bundle; `get_key_moment` fallback-сканирует legacy JSONL и backfill-ит per-moment index; сортировка сессий нормализует legacy naive + UTC-aware timestamp |
...
| `adapters/memory/in_memory_memory_guardian.py` (`InMemoryMemoryGuardian`) | `MemoryGuardian` | scan_orphan_entities + scan_merge_candidates + scan_stale_moments + scan_embedding_gaps + жизненный цикл findings; **HLE-31** также реализует `scan_quality_metrics` — sliding-window проверки: `affect_detector_silent` (incomplete_coloring rate), `divergence_pattern` (одинаковый divergence_type ≥ N через опциональный `DivergenceEventStore`), `stance_formation_too_fast` (stance сформирован < 24h после backing моментов через опциональный `EntityStanceStore`). Отсутствие optional входов ⇒ под-скан молча возвращает 0 findings. **HLE-32** также реализует inline пост-write проверки: фактическое отсутствие `metadata['embedding']` → `embedding_missing`; entity без embedding (кроме value/principle) → `embedding_missing`; KeyMoment с `incomplete_coloring=True` → `affect_detector_silent` (severity `info`). Все inline-проверки делают dedup по `(target_table, target_id, finding_type)` среди unresolved finding'ов |
...
| `adapters/reflection/state_store_session_repository.py` (`StateStoreSessionRepository`) | `SessionRepository` | тонкий адаптер над любым `StateStore` (InMemory / File / Postgres v2); default `agent_id` через конструктор для single-agent + явная трёхаргументная форма для multi-agent; нормализует границы диапазона и legacy naive timestamp сессий/моментов в UTC, предупреждает при saturation client-side fetch cap; фундамент для R3+R4 (миграция Daily/Deep reflection) |
...
| `adapters/storage/reflection_store.py` (`ReflectionStore`) | — | PostgreSQL `agent_{N}.reflections` через `AgentSchemaResolver` (без RLS); **переехал сюда из `atman/reflection/store.py` в HLE-53 — старый пакет распущен, persistence-модель живёт в `adapters/storage/reflection_persistence_models.py` и переиспользует канонический `core.models.reflection.ReflectionLevel`** |
| `adapters/storage/_atomic_write.py` (`write_atomically`) (HLE-49) | — | Общий хелпер tempfile + fsync + chmod + os.replace; используется `FileBackend._save_facts` и `FileStateStore._write_json_atomically`, чтобы контракт атомарной записи жил в одном месте |
| `adapters/clock.py` (`FrozenClock`) (HLE-54) | `ClockPort` | Замороженные часы для тестов; дефолтный `SystemClock` живёт в `core/clock_impl.py`, так как это zero-dep обёртка над stdlib |
...
| `adapters/agent/deps.py` (`AtmanDeps`, `AtmanDeps.from_config`) | — | замороженный DI-контейнер, связывающий `SessionManager`, `IdentityService`, `ExperienceService`, `MicroReflectionService`, `StateStore`; фабрика `from_config` переносит валидированные лимиты из `AgentConfig`; опциональное поле `injected_context` для режима `system_prompt`; **R11.7/R12** опциональные поля `pending_review_inbox` и `reflection_request_queue`, определяющие регистрацию соответствующих инструментов; **opt-2** опциональное поле `passive_memory_injector`; **WP-08 v2** опциональное поле `skill_manager: SkillManagerPort | None = None`; **HLE-29** опциональное поле `divergence_event_store: DivergenceEventStore | None = None`, открывающее персистированный поток divergence-событий для R6 DivergenceAggregator (и любых ad-hoc читателей) без доступа во внутренности `SessionManager`; **HLE-30** опциональные `reflection_overload_monitor: ReflectionOverloadMonitor | None = None` + `overload_alert_inspect: InMemoryOverloadAlertSink | None = None` (in-memory отвод composite sink'а; admin UI / debug-точки читают `.alerts` без захода в приватный `_sink._sinks`); **HLE-32** опциональное `memory_guardian: MemoryGuardian | None = None` — тот же инстанс, что и у inline validator'а, exposed для cli_maintenance / admin без пересборки цепочки; **HLE-33** опциональное `ambient_memory: AmbientMemoryService | None = None` — entity-anchor параллельный RAG, вызывающие зовут `compose_injection(text, agent_id=...)` напрямую через deps |
...
| `skills/models.py` | Доменные модели (замороженные dataclasses + StrEnum) | `SkillKind`, `SkillStatus`, `SkillOrigin`, `Skill`, `SkillInvocation`, `SuggestionStrength`, `SkillSuggestion`, `DailySkillSummary`, `DeepSkillSummary` (возвраты HLE-36 хуков); свойства `Skill.is_pinned`, `Skill.description_short` |
...
| `skills/port.py` | `SkillManagerPort` Protocol (runtime_checkable) | `SkillManagerPort`: 11 методов — `list_pinned`, `list_available`, `trigger_router`, `invoke`, `mark_result`, `capture`, `get_skill`, `process_session_skills`, `write_session_skills_marker` (HLE-35), `process_daily_skills` (HLE-36), `process_deep_skills` (HLE-36) |
| `skills/noop.py` | Тихая заглушка для отключённого режима | `SkillsDisabledError`, `NoopSkillManager` (read-only методы возвращают пустые коллекции; write-методы выбрасывают `SkillsDisabledError`; `process_session_skills`, `write_session_skills_marker`, `process_daily_skills`, `process_deep_skills` — тихие no-op, возвращают пустые значения/None) |
...
| `skills/manager.py` | Реализация `SkillManager` | `SkillManager`: полный lifecycle — invoke (subprocess entry script, preliminary_status), mark_result, capture (создаёт SKILL.md + entity + row), `process_session_skills` (финализирует invocations, обновляет статистику, auto-pin/downgrade, помечает processed), `write_session_skills_marker` (HLE-35: пишет `atman_session_skills_<ts>.json` в workspace), `process_daily_skills` (HLE-36: bump revision_priority для долго неиспользуемых pending-скиллов, список high-priority кандидатов), `process_deep_skills` (HLE-36: read-only классификация — archive-кандидаты по порогу простоя, problematic по failure rate) |
| `skills/text_parser.py` | HLE-37 fallback: парсинг ответа агента на неявные упоминания скиллов | `InvocationTextParser(store, min_confidence=0.7, max_per_call=5)`; `parse(text, agent_id, session_id) -> list[InferredInvocation]` (pure), `parse_and_record(...)` (создаёт invocations с `agent_marker='unclear'`); скоринг: точное имя (1.0), вариант kebab→underscore/space (0.9), ≥2 keyword overlap (0.9), один keyword (0.75); word-boundary match исключает ложные срабатывания |
| `skills/installer.py` | HLE-38 установщик install-external (pure) | `install_external(source, agent_id, *, store, agents_root, name_override=None, dry_run=False, http_get=None)`; источники: локальная папка / локальный `.zip` / HTTPS `.zip`; валидирует манифест, отказывает zip-slip + plain HTTP + bare GitHub URL; `SkillInstallError`, `InstallResult` |
| `skills/revision.py` | HLE-40 workflow ревизии скиллов | `SkillReviser` Protocol + `NoopSkillReviser` по умолчанию; `SkillRevisionProposal`, `RevisionOutcome`; `SkillRevisionService(store, reviser).revise_pending(agent_id, max_skills=3, dry_run=False)` — сохраняет бэкап `SKILL.md` как `SKILL.md.bak.<UTC-ts>`, переписывает body, bump минорной версии, сбрасывает `revision_needed`; `_bump_minor_version(version)` helper |
...
**Конфиг (HLE-36)**: `SkillsSettings.daily_revision_idle_bump_sessions` (по умолчанию 5), `deep_archive_sessions` (по умолчанию 50), `deep_failure_rate_threshold` (по умолчанию 0.5), `deep_min_invocations_for_failure_rate` (по умолчанию 5).

**Override через env (HLE-41)**: `ATMAN_SKILLS_ENABLED=false|0|no|off` короткозамыкает `factory._build_skill_manager`, имеет приоритет над `settings.skills.enabled`.
...
| `src/atman/skills/cli.py` | CLI | Управление навыками: `list`, `show`, `disable`, `enable`, `pin`, `unpin`, `archive`, `inspect-invocations`, `force-revise`, `install-external` (HLE-38), `revise` (HLE-40); read-only команды работают при `skills.enabled=false`; entry point `atman-skills` |
...
| `DeepReflectionService` ↔ `MergeCandidatesHandler` (R10) | `core/services/merge_candidates_handler.py` → `core/ports/memory_guardian.py` (`get_unresolved` / `resolve_finding`), `core/ports/entity_registry.py` (`get_entity` / `merge_entities`), `core/ports/state_store.py` (`find_moments_by_entity`), `core/ports/reflection.py` (`ReflectionModel.decide_entity_merge` + `SYSTEM_PROMPT_ENTITY_MERGE`) | опциональный хук (требует `agent_id`); для каждого unresolved `similar_entities` finding (отложенного R8), спрашивает LLM с ≤ `DEFAULT_CONTEXT_LIMIT` (5) недавними KeyMoments per side. Confirm → `EntityRegistry.merge_entities` (FK rewrite на стороне registry) + `resolve_finding(fixed)`. Reject с reason → `resolve_finding(ignored, note=reason)`. Любая ошибка (LLM/store/registry) или пустой reason → finding остаётся unresolved для следующего Deep — никогда half-merge. Keep-side выбирается по `canonical_name` от LLM, иначе по `mention_count`. |
| `DeepReflectionService._propose_identity_revision` ↔ R5/R7/R8/R9/R10 outcomes (R11) | `core/services/reflection_service.py` | R11 identity-level synthesis: в дополнение к существующим `potential_habit`/`potential_principle` и низкому `health.overall_score`, proposal теперь читает описания marker-паттернов (`growth_indicator=progress/regression`, `agency_level=high/low`) и outcomes от R7 stance revise, R9 relations, R10 merge, R8 triage. Output остаётся free-form proposal — реальные writes идут через R11.5 `IdentityService.apply_self_change` + `SelfAppliedChangeStore` audit. |
...
| Связка | Файлы | Тип |
|--------|-------|-----|
| `AtmanDeps` ↔ `SessionManager`, `IdentityService`, `ExperienceService`, `MicroReflectionService`, `StateStore` | `adapters/agent/deps.py` | DI-контейнер (frozen dataclass); опциональный `injected_context` для режима `system_prompt` внедрения памяти; **WP-08 v2** опциональное `skill_manager: SkillManagerPort | None`; **HLE-29** опциональное `divergence_event_store: DivergenceEventStore | None` (тот же инстанс, что и в affect-pipeline — читатели видят то, что было записано); **HLE-30** опциональные `reflection_overload_monitor: ReflectionOverloadMonitor | None` + `overload_alert_inspect: InMemoryOverloadAlertSink | None`; **HLE-32** опциональное `memory_guardian: MemoryGuardian | None` (общий инстанс с inline validator); **HLE-33** опциональное `ambient_memory: AmbientMemoryService | None` |
...
- ✅ Legacy JSONL-only строки `KeyMoment` в `FileStateStore` — `get_key_moment` сканирует `key_moments.jsonl`, пропускает битые строки и backfill-ит `{id}.json`; покрыто `tests/test_file_state_store.py::test_get_key_moment_loads_legacy_jsonl_and_backfills_index_file` и `test_search_experiences_uses_legacy_jsonl_moment_fallback`.
- ✅ Смесь legacy naive + UTC-aware timestamp у сессий — store-level сортировка `list_recent_sessions` и repository range filtering нормализуют в UTC; покрыто `tests/test_file_state_store.py::test_list_recent_sessions_sorts_mixed_legacy_naive_and_aware_timestamps` и `tests/test_state_store_session_repository.py::test_get_sessions_in_range_normalizes_legacy_naive_session_timestamps`.
...
| текущий PR | `list_recent_sessions` в file/in-memory store мог падать с `TypeError` до repository UTC-нормализации, если legacy naive и current aware timestamps сосуществовали | исправлено: sort key использует `ensure_utc`; покрыто `tests/test_file_state_store.py::test_list_recent_sessions_sorts_mixed_legacy_naive_and_aware_timestamps` и `tests/test_state_store_session_repository.py::test_get_sessions_in_range_normalizes_legacy_naive_session_timestamps` |
| HLE-27/HLE-28/HLE-30 follow-up | File-backed lingvo jobs могли завершаться успешно без сохранения `structured_markers`; mREBEL мог привязать relation к одноимённой сущности неправильного типа; PostgreSQL `maintenance_jobs` отклонял `reflection_overload_check` | покрыто (`tests/test_post_write_wiring.py::test_worker_lingvo_persists_structured_markers_in_file_store`, `tests/test_post_write_wiring.py::test_worker_mrebel_resolves_entities_by_detected_type`, `tests/test_state_store_contract.py::test_update_moment_structured_markers_does_not_raise`, `tests/test_postgres_migration_security.py::test_maintenance_job_migrations_accept_all_job_names`) |
...
| Legacy KeyMoment JSONL fallback + сортировка смешанных timestamp сессий | ✅ закрыто | `tests/test_file_state_store.py`, `tests/test_state_store_session_repository.py` |
