...
| `core/clock_impl.py` | `ensure_utc(dt)` — чистый нормализатор временной зоны; конкретные `SystemClock` / `FrozenClock` переехали в `adapters/clock.py` (HLE-54) |
...
| `adapters/clock.py` (`SystemClock`, `FrozenClock`) (HLE-54) | `ClockPort` | Wall-clock + замороженные часы для тестов; переехали из `core/clock_impl.py`, чтобы Core больше не содержал конкретных реализаций |
