...
По возможности использовать **[uv](https://github.com/astral-sh/uv)** для окружения и запуска: `uv venv`, `uv pip install -e ".[dev]"`, **`uv run`** для `python`, `pytest`, `ruff`, `pyright` и других dev-команд — так проще воспроизводить шаги и не смешивать глобальный Python. Подробности и примеры — в **`AGENTS.md`** (раздел *uv — рекомендуемый workflow*). `pip` допустим, если `uv` недоступен.
...
2. **Воспроизводимый сценарий** — одна команда или короткая последовательность
   ...
   вывод демо и CLI — по **`AGENTS.md`** (*Пользовательский вывод в терминале (Rich)*):
