...
1. **Модели**:
   ...
   - `HealthAssessment` — проверка психологического здоровья (6 критериев Яходы)
...
2. **Сервисы**:
   ...
   - `NarrativeRevisionService` — обновление нарратива
...
3. **Deep Reflection**: Оценка здоровья по 6 критериям Яходы
...
- Документ `Self-Narrative`
- Существующие записи `Uncertainty`

### Что пишет рефлексия

- `reframing_notes` к существующему опыту (append-only)
- Записи `ReflectionEvent`
- Обнаружения `PatternCandidate`
- Обновленные или новые записи `Uncertainty`
- Черновики изменений `Identity` и `Narrative`
...
## Оценка здоровья (6 критериев Яходы)

Deep reflection оценивает психологическое здоровье по критериям Jahoda:
...
Micro обновляет checkpoint, Daily обнаруживает паттерны, Deep пересматривает идентичность.

**Почему**: Разделение ответственности — частые легкие обновления vs редкие глубокие ревизии.
...
Micro обновляет recent layer, Deep обновляет core layer.
