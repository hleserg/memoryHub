...
После `pip install -e ".[dev]"` из корня репозитория:

```bash
make demo-experience
```

Эквивалент: `python3 src/demo_experience_store.py`. Используется **временный** JSONL-файл и [`fixtures/experience1_competence_challenge.json`](../../../fixtures/experience1_competence_challenge.json); каталог `~/.atman` не изменяется.
