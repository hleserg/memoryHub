...
def test_search_functionality(backend):
    ...
    backend.add_fact(FactRecord(content="Важная задача", source="test", tags=["task", "important"]))
    backend.add_fact(FactRecord(content="Обычное сообщение", source="test", tags=["info"]))
