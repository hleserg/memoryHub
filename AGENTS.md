...
- **Обязательно Rich** для: интерактивных CLI (`src/atman/cli.py`, `src/atman/cli_experience.py`), воспроизводимых демо (`src/demo.py`, `src/demo_experience_store.py`, новые `src/demo_*.py`, цели `make demo-*`), любых новых entrypoint’ов с пошаговым или табличным выводом.
...
- **Пошаговое раскрытие демо:** цели **`make demo-factual`** и **`make demo-experience`** по умолчанию задают **`ATMAN_DEMO_PACE=1`** (~0.45 с между крупными блоками). Для **мгновенного** вывода: **`make demo-factual-fast`** / **`make demo-experience-fast`** или `ATMAN_DEMO_PACE=off python3 src/demo….py`. Иначе env **`ATMAN_DEMO_PACE`** можно выставить вручную (`0.6`, максимум 3 с). В коде демо вызывайте **`atman.term.demo_pace()`** после баннеров/секций/больших таблиц. Цели **`demo-*-paced`** — синонимы обычных `demo-*`.
...
2. **Runnable scenario** — e.g. `make demo-factual`, `make demo-experience`, or `python3 src/demo*.py`, documented here and in the PR template. Console output for demos must use **Rich** via **`atman.term`** (see *Пользовательский вывод в терминале (Rich)* above).
