...
Atman — проект психологического слоя для AI-агента, находящийся на стадии **прототипирования**. Содержит документацию (markdown-файлы, изображения, шаблоны) и реализованные компоненты Python-пакета `atman`: **Factual Memory Adapter**, **Experience Store** (WP-02), **Identity Store** (WP-03), **Reflection Engine** (WP-04).
...
- **Обязательно Rich** для: интерактивных CLI (`src/atman/cli.py`, `src/atman/cli_experience.py`, `src/atman/cli_identity.py`, `src/atman/cli_reflection.py`), воспроизводимых демо (`src/demo.py`, `src/demo_experience_store.py`, `src/demo_identity.py`, `src/demo_reflection.py`, `src/demo_web_dashboard.py`, новые `src/demo_*.py`, цели `make demo-*`), любых новых entrypoint’ов с пошаговым или табличным выводом.
...
1. **Feature narrative** — work package under `docs/development/work-packages/` and/or paired guides under `docs/features/<slug>/` (e.g. [`docs/features/factual-memory/README.md`](docs/features/factual-memory/README.md), [`docs/features/experience-store/README.md`](docs/features/experience-store/README.md), [`docs/features/reflection-engine/README.md`](docs/features/reflection-engine/README.md) + `README-ru.md`).
2. **Runnable scenario** — e.g. `make demo-factual`, `make demo-experience`, `make demo-reflection`, or `python3 src/demo*.py`, documented here and in the PR template. Console output for demos must use **Rich** via **`atman.term`** (see *Пользовательский вывод в терминале (Rich)* above).
