...
- **Обязательно Rich** для: интерактивных CLI (`src/atman/cli.py`, `src/atman/cli_experience.py`, `src/atman/cli_identity.py`), воспроизводимых демо (`src/demo.py`, `src/demo_experience_store.py`, `src/demo_identity.py`, `src/demo_web_dashboard.py`, новые `src/demo_*.py`, цели `make demo-*`), любых новых entrypoint’ов с пошаговым или табличным выводом.
