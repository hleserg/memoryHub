...
- `atman_agent_cli/` — опциональный **coding-agent** (Textual/RAG), отдельно от wheel ядра; `pip install -e ".[agent-cli]"`, `PYTHONPATH=atman_agent_cli/src:src`, подготовка `make agent-check-prep`, операторский разбор [`atman_agent_cli/RUNBOOK.md`](atman_agent_cli/RUNBOOK.md); ядро **не импортирует** `atman.agent_cli` (см. `.importlinter`)
...
- `src/demo_eval_runner.py` — walkthrough E1 Evaluation Runner: registry list → RunnerCore + JsonlReporter → noop benchmark → idempotent rerun (`make demo-eval-runner`; см. `docs/features/eval-runner/README.md`)
...
- **Обязательно Rich** для: интерактивных CLI (`src/atman/cli.py`, `src/atman/cli_experience.py`, `src/atman/cli_identity.py`, `src/atman/cli_reflection.py`), воспроизводимых демо (`src/demo.py`, `src/demo_experience_store.py`, `src/demo_identity.py`, `src/demo_reflection.py`, `src/demo_session_manager.py`, `src/demo_web_dashboard.py`, `src/demo_eval_runner.py`, новые `src/demo_*.py`, цели `make demo-*`), любых новых entrypoint’ов с пошаговым или табличным выводом.
...
- **Пошаговое раскрытие демо:** цели **`make demo-factual`**, **`make demo-experience`**, **`make demo-identity`**, **`make demo-reflection`**, **`make demo-session`**, **`make demo-full-corpus`**, **`make demo-webui`**, **`make demo-eval-runner`** по умолчанию задают **`ATMAN_DEMO_PACE=1`** (для **`demo-full-corpus`** Makefile также выставляет **`PYTHONPATH=.`** для импорта пакета **`e2e`**) (~0.45 с между крупными блоками). Для **мгновенного** вывода: **`make demo-*-fast`** или `ATMAN_DEMO_PACE=off python3 src/demo….py`. Иначе env **`ATMAN_DEMO_PACE`** можно выставить вручную (`0.6`, максимум 3 с). В коде демо вызывайте **`atman.term.demo_pace()`** после баннеров/секций/больших таблиц. Цели **`demo-*-paced`** — синонимы обычных `demo-*`.
...
- **Coding agent CLI (optional)** — см. **`atman_agent_cli/`**: `uv pip install -e ".[dev,agent-cli]"`; `PYTHONPATH=atman_agent_cli/src:src`; подготовка без pytest: **`make agent-check-prep`**; mock LLM: **`make agent-mock-llm`** (во втором терминале); подробнее [`atman_agent_cli/RUNBOOK.md`](atman_agent_cli/RUNBOOK.md)
...
- **Demo (Eval Runner)**: `make demo-eval-runner` or `python3 src/demo_eval_runner.py`; мгновенно: `make demo-eval-runner-fast`
- **Eval list**: `make eval-list` or `python3 -m atman.eval.benchmark_runner list`
- **Eval run**: `make eval-run` or `python3 -m atman.eval.benchmark_runner run <key>`
