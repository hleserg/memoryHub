...
class ReflectionRecord(BaseModel):
    ...
    - session_refs: UUID[] NOT NULL DEFAULT '{}' -- sessions analyzed (renamed from experience_refs by R15)
    ...
    session_refs: list[UUID] = Field(
        default_factory=list,
        description=(
            "UUIDs of sessions analyzed in this reflection (R15 — was 'experience_refs'; "
            "one virtual SessionExperience per Session, so ids match by construction)"
        ),
    )
    ...
    @field_validator("session_refs", "reframing_note_ids")
    @classmethod
