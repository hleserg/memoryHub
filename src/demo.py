...
def demo_in_memory():
    ...
    fact2 = memory.add_fact(
        FactRecord(
            content="Factual memory adapter реализован", source="session_002", tags=["task", "done"]
        )
    )
    print(f"   ✓ Добавлен факт: {fact2.id}")

    fact3 = memory.add_fact(
        FactRecord(content="Все тесты прошли успешно", source="session_002", tags=["test", "done"])
    )
...
def demo_file_backend():
    ...
    if retrieved:
        print("   ✓ Факт успешно загружен из файла:")
